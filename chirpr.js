...
let insert_chirp_func = function(message_and_context, has_parent = false,
  player_reply = null) {
  ...
  if (null != player_reply) {
    ...
    player_reply_chirp_avatar.innerHTML = getReplyName(chirp_grammar);
  ...
  if (false == has_parent) {
    ...
    //}
    last_chirp = document.getElementById("timeline").insertBefore(new_chirp,
      last_chirp);

  } else {
    let nesting_level = parseInt(has_parent.parentNode.dataset.nesting);
    new_chirp.className = new_chirp.className + " reply-to-chirp " + (0 == (
      nesting_level % 2) ? "nesting-even" : "nesting-odd");;
    ...
    if (null != player_reply_chirp) {
      ...
      has_parent.parentNode.insertBefore(player_reply_chirp, has_parent.parentNode
        .getElementsByClassName("send-button")[0].nextSibling);
    } else {
      has_parent.parentNode.insertBefore(new_chirp, has_parent.parentNode.getElementsByClassName(
        "send-button")[0].nextSibling);
    }
  }
  ...
  reply_button.addEventListener("click", function() {
    ...
    let respondToClick = function() {
      ...
      if (("" != output_text.message) && (null != output_text.message) &&
        (undefined != output_text.message)) {
